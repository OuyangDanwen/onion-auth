...
public class Sender {
	...
	private int receivedDHPubKeySize;
		...
		try {
			...
			if (verifySignature()) {
				...
				if (compareSecretKeyHashes(hashedSecretKey, receivedHashedSecretKey)) {
					System.out.println("Sender: Hashes of secret keys are a match!");
				} else {
					System.out.println("Sender: Hashes of secret keys do not match");
				}
			} else {
	...
	private void receiveMessageWithSignature() throws Exception {
		receivedDHPubKeySize = fromReceiver.readInt(); // need to explicitly state the received DH pub key size as it varies by a degree of 1 byte occassionally

		byte[] rawReceivedMessage = new byte[1024];
		int rawReceivedMessageLength = fromReceiver.read(rawReceivedMessage, 0, rawReceivedMessage.length);
		receivedMessage = new byte[rawReceivedMessageLength];
		System.arraycopy(rawReceivedMessage, 0, receivedMessage, 0, receivedMessage.length);

		System.out.println("Sender: receivedMessage.length = " + receivedMessage.length);
	...
	// 3. Hash value of the secret key generated by the other peer
	private void extractDataFromMessage() throws Exception {
		int a = receivedDHPubKeySize; // used as reference for the length 
		...
		receivedHashedSecretKey = Arrays.copyOfRange(receivedMessage, a+b, receivedMessage.length);
	...
	private boolean compareSecretKeyHashes(byte[] hashedSecretKey, byte[] receivedHashedSecretKey) {
		System.out.println("Sender: Comparing secret key hashes");
